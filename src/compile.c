...
u8 *machine_compile(machine_t *m, str_t source) {
    ...
    // printf("The length of the source is: %zu\n", length);
    printf("PC; %lx ,The length of the source is: %zu\n",m->state.pc, length);
    ...
    if (rela_idx == 0 || rodata_idx == 0) {
        jit_size +=text_shdr->sh_size;
        
        printf("jit_code_size:%zu\n",jit_size);
